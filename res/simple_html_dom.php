 ...
 * @subpackage Scrappers_project_AASTU_simple_html_dom
 */
...
function Scrappers_project_AASTU_file_get_html($url, $use_include_path = false, $context=null, $offset = 0, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
	// Ensure maximum length is greater than zero
	if($maxLen <= 0) { $maxLen = MAX_FILE_SIZE; }

	// We DO force the tags to be terminated.
	$dom = new Scrappers_project_AASTU_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
function Scrappers_project_AASTU_str_get_html($str, $lowercase=true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
	$dom = new Scrappers_project_AASTU_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
// dump html dom tree
function Scrappers_project_AASTU_dump_html_tree($node, $show_attr=true, $deep=0)
{
	$node->dump($node);
}
 ...
 * @package PlaceLocalInclude
 */
class Scrappers_project_AASTU_simple_html_dom_node
{
	/**
	 * Node type
	 *
	 ...
	 *
	 * Adds itself to the list of DOM Nodes {@see Scrappers_project_AASTU_simple_html_dom::$nodes}
	 */
 ...
 * @package PlaceLocalInclude
 */
class Scrappers_project_AASTU_simple_html_dom
{
	/**
	 ...
	 * string length of ({@see Scrappers_project_AASTU_simple_html_dom::$doc}).
	 *
	 ...
	 * Current character
	 *
	 * Holds the current character at position {@see Scrappers_project_AASTU_simple_html_dom::$pos} in
	 * the document {@see Scrappers_project_AASTU_simple_html_dom::$doc}
	 *
	...
	protected function prepare($str, $lowercase=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new Scrappers_project_AASTU_simple_html_dom_node($this);
		$this->root->tag = 'root';
	...
	protected function parse()
	{
		while (true) {
			...
			$node = new Scrappers_project_AASTU_simple_html_dom_node($this);
			++$this->cursor;
	 ...
	 *
	 * Noise is stored to {@see Scrappers_project_AASTU_simple_html_dom::$noise}
	 *
	 ...
	 *
	 * Noise is restored from {@see Scrappers_project_AASTU_simple_html_dom::$noise}
	 *
	...
	function createElement($name, $value=null) {return Scrappers_project_AASTU_str_get_html("<$name>$value</$name>")->first_child();}
	function createTextNode($value) {return end(Scrappers_project_AASTU_str_get_html($value)->nodes);}
	function getElementById($id) {return $this->find("#$id", 0);}
	function getElementsById($id, $idx=null) {return $this->find("#$id", $idx);}
	function getElementByTagName($name) {return $this->find($name, 0);}
