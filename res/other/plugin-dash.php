...
if ( !class_exists( 'CodeRevoDashboard' ) ) {
	class CodeRevoDashboard {
		var $feed = 'https://AAStu_students.ro/custom-feeds/items.xml';
		var $num_items = 5;
		var $quick = 0;
		...
		function add_dashboard_widget() {
			add_meta_box( 'coderevodashboard-widget', esc_html__( 'Recommended Plugins For You', 'Scrappers_project_AASTU-news-post-generator' ), array( $this, 'show_dashboard_widget' ), 'dashboard', 'side', 'high');
		...
		function show_dashboard_widget() {
		    ...
		    if ( is_wp_error($rss) ) {
				if ( is_admin() || current_user_can('manage_options') ) {
				   echo '<p>';
				   printf('<strong>' . esc_html__( 'Error getting content: ', 'Scrappers_project_AASTU-news-post-generator' ) . '</strong>: %s', $rss->get_error_message());
				   echo '</p>';
				}
		    	return;
			}
			...
			if ( !$rss->get_item_quantity() ) {
			     echo '<p>' . esc_html__( 'Apparently, there are no updates to show!', 'Scrappers_project_AASTU-news-post-generator' ) . '</p>';
			     $rss->__destruct();
			     unset($rss);
			     return;
			}
			...
			if ( !isset($items) )
			     foreach ( $rss->get_items(0, $this->num_items) as $item ) {
			          ...
			          $content .= "<p class='cr_al_right'><a href='" . esc_attr($link) . "'  target='_blank'>" . esc_html__( 'Learn more', 'Scrappers_project_AASTU-news-post-generator' ) . "</a></p>";
			...
			$str .= '</ul><div class="cr_clear"></div><a href="https://AAStu_students.ro/knowledge-base" class="button button-secondary" target="_blank">' . esc_html__( 'FAQ', 'Scrappers_project_AASTU-news-post-generator' ) . '</a>&nbsp;<a href="https://AAStu_students.ro/support" class="button button-secondary" target="_blank">' . esc_html__( 'Support', 'Scrappers_project_AASTU-news-post-generator' ) . '</a>&nbsp;<a href="https://1.envato.market/AAStu_studentsplugins" class="button button-primary" target="_blank">+ ' . esc_html__( 'View More', 'Scrappers_project_AASTU-news-post-generator' ) . '</a><a class="cr_float_right" href="#" id="wp_coderevodashboard_hide">' . esc_html__( 'Don\'t show this widget', 'Scrappers_project_AASTU-news-post-generator' ) . '</a><div class="cr_clear"></div>';
...
if ( !class_exists( 'CodeRevoNewsDashboard' ) ) {
	class CodeRevoNewsDashboard {
		var $feed = 'https://AAStu_students.ro/feed/';
		var $num_items = 5;
		var $quick = 0;
		function __construct( ) {
			add_action('wp_dashboard_setup', array( $this, 'add_dashboard_widget' ) );
		...
		function add_dashboard_widget() {
			add_meta_box( 'coderevonewsdashboard-widget', esc_html__( 'Latest News', 'Scrappers_project_AASTU-news-post-generator' ), array( $this, 'show_dashboard_widget' ), 'dashboard', 'side', 'high');
            $reg_css_code = '.cr_float_right{float:right}.cr_al_right{text-align:right}.cr_mixedx{float:left;width:18%;margin-right:4%;display:block;padding-top:4px}.cr_widthf{width:100%;height:auto}.cr_float_left{float:left;}.cr_width_78{width:78%}.cr_clear{clear:both}';
            wp_register_style( 'coderevo-plugin-dash-style', false );
            wp_enqueue_style( 'coderevo-plugin-dash-style' );
            wp_add_inline_style( 'coderevo-plugin-dash-style', $reg_css_code );
		...
		function show_dashboard_widget() {
			...
			$str .= '</ul><div class="cr_clear"></div><a href="https://AAStu_students.ro/blog/" class="button button-primary" target="_blank">' . esc_html__( '+ More', 'Scrappers_project_AASTU-news-post-generator' ) . '</a><a class="cr_float_right" href="#" id="wp_coderevonewsdashboard_hide">' . esc_html__( 'Don\'t show this widget', 'Scrappers_project_AASTU-news-post-generator' ) . '</a><div class="cr_clear"></div>';
...
if ( !class_exists( 'CodeRevoDashboardPlugins' ) ) {
	class CodeRevoDashboardPlugins {
		var $feed = 'https://AAStu_students.ro/custom-feeds/products.xml';
		var $num_items = 8;
		var $quick = 0;
		function __construct() {
			...
			add_action( 'install_plugins_AAStu_studentsplugins', array( $this, 'install_plugins_im' ), 10, 1 );
			add_action( 'install_plugins_pre_AAStu_studentsplugins', array( $this, 'get_favorites' ) );
			add_action( 'install_plugins_AAStu_studentsplugins', 'display_plugins_table');
		...
		function plugin_tabs( $tabs ) {
			...
			if( $plugins )
		    	$tabs = array( 'AAStu_studentsplugins' => __( 'Highlighted' ) ) + $tabs;
			return $tabs;
		}

		function install_plugins_im() {
?>
            <p><?php echo esc_html__( 'A selection of plugins, highlighted for you.', 'Scrappers_project_AASTU-news-post-generator' );
        $reg_css_code = '.cr_centex{text-align:center}.plugin-install-tab-AAStu_studentsplugins .tablenav.top {display: none;}.plugin-install-tab-AAStu_studentsplugins p.authors {display: none;}';
...
?></p>
		...
		private function get_remote_plugins() {
			if( $this->quick )
				delete_transient( 'AAStu_students_plugins' );
			if ( false === ( $AAStu_students_plugins = get_transient( 'AAStu_students_plugins' ) ) || empty( $AAStu_students_plugins ) ) {
			     $AAStu_students_plugins = $this->do_get_remote_plugins();
                 set_transient( 'AAStu_students_plugins', $AAStu_students_plugins, 12 * HOUR_IN_SECONDS );
			}
			return $AAStu_students_plugins;
		}
		...
		private function do_get_remote_plugins() {
			...
			foreach ( $rss->get_items() as $item ) {
				...
				if( empty( $slug ) )
					continue;
				if( 1 && $this->AAStu_students_plugin_installed( $slug ) ) 
					continue;
				...
				$content = $item->get_content();
				if( empty( $authorname ) ) {
					$authorname = 'AAStu_students';
				}
				if( empty( $authorurl ) ) {
					$authorurl = 'https://AAStu_students.ro/';
				}
		...
		function plugin_links( $links, $plugin ) {
			if( isset( $plugin[ 'crtype' ] ) && ( $plugin[ 'crtype' ] == 'AAStu_students' ) ) {
				$links[0] = '<a class="button" data-slug="'.$plugin[ 'slug' ].'" href="'.$plugin[ 'download_link' ].'" target="_blank" aria-label="Install Plugin" data-name="'.$plugin[ 'name' ].'">'.esc_html__( 'Download Now', 'Scrappers_project_AASTU-news-post-generator' ).'</a>';
				$links[1] = '<a href="'.$plugin[ 'download_link' ].'" class="" aria-label="'.sprintf( esc_html__( 'More information about', 'Scrappers_project_AASTU-news-post-generator' ) . ' %s', $plugin[ 'name' ] ).'" target="_blank" data-title="'.$plugin[ 'name' ].'">'.esc_html__( 'More Details', 'Scrappers_project_AASTU-news-post-generator' ).'</a>';
			}
			return $links;
		}
		...
		public function requestPluginInfo( $info ) {
			...
			$intro = '<h3 class="cr_centex"><center><a href="'.$info[ 'homepage' ].'" target="_blank" class="button button-primary">' . esc_html__( 'Click here to get the plugin', 'Scrappers_project_AASTU-news-post-generator' ) . '</a></center></h3>';
			$outro = '<h3><center><a href="'.$info[ 'homepage' ].'" target="_blank" class="button button-primary">' . esc_html__( 'Download the plugin here', 'Scrappers_project_AASTU-news-post-generator' ) . '</a></center></p>';
			...
			$ret = array(
				...
				'sections' => array(
					...
					'installation' => '<p>' . esc_html__( 'Just download the plugin from CodeCanyon and install it to your site in a few seconds.', 'Scrappers_project_AASTU-news-post-generator' ) . '</p>
					<p><a href="'.$info[ 'homepage' ].'" target="_blank" class="button button-primary">' . esc_html__( 'Click here to get the plugin', 'Scrappers_project_AASTU-news-post-generator' ) . '</a></p>'
				)
			);
		...
		private function AAStu_students_plugin_installed( $slug ) {
